...
// * LoveIcon
export const LoveIcon: React.FC<{
	alreadyLoved?: boolean;
	height?: string;
}> = memo(({ alreadyLoved = false, height = "24" }) => {
	const [isLoved, setIsLoved] = useState(alreadyLoved);

	const { love } = useSpring({
		from: { love: 0 },
		love: isLoved ? 1 : 0,
		config: { duration: 200 },
	});
...
const ArticleActionBar: React.FC<Props> = memo(({ multipleImage, isLoved }) => {
	return (
		<section
			className='article__actionBar'
			style={{ marginTop: multipleImage ? "-1.2em" : 0 }}>
			<LoveIcon alreadyLoved={isLoved} />
			<span className='article__actionBar__icon__container'>
				<button className='article__actionBar__icon__container__button'>
					<svg
						aria-label='Comment'
